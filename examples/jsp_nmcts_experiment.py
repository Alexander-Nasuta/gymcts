...
class ExperimentWrapper(DeepCopyMCTSGymEnvWrapper, gym.Wrapper):
    ...
    def step(
        ...
        ExperimentNmctsAgent.visited_states_set.add(tuple(obs.ravel()))

        if term or trun:
            log.info(
                f"num_timesteps: {ExperimentWrapper.performed_timesteps}, "
                f"Makespan: {self.env.unwrapped.get_makespan()}"
                f", visited_states: {len(ExperimentNmctsAgent.visited_states_set)}"
            )
